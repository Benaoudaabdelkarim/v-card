...
class VcardAPIController extends AppBaseController
{
    ...
    public function getVcardBasicDetails(Vcard $vcard)
    {
        ...
        $vcardData = [
            'profile_url' => $vcard->profile_url ?? null,
            'cover_url' => $vcard->cover_url ?? null,
            'url_alias' => $vcard->url_alias,
            'name' => $vcard->name,
            'occupation' => $vcard->occupation ?? null,
            'description' => $vcard->description ?? null,
            'first_name' => $vcard->first_name ?? null,
            'last_name' => $vcard->last_name ?? null,
            'email' => $vcard->email ?? null,
            'phone' => $vcard->phone ?? null,
            'region_code' => $vcard->region_code ?? null,
            'default_language' => $vcard->default_language ?? null,
            'language_enabled' => $vcard->language_enable ?? null,
        ];

        return $this->sendResponse($vcardData, 'Vcard basic details retrieved successfully.');
    }
